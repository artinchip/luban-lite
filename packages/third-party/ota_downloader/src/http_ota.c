    ...
    switch (aic_get_boot_device()) {
        ...
        case BD_SPINOR:
            ...
            if (fal_partition_erase(dl_part, 0, dl_part->len) < 0) {
                LOG_E(
                    "Firmware download failed! Partition (%s) erase error! len = %d",
...
#ifdef AIC_SPINOR_DRV
        case BD_SPINOR:
            if (fal_partition_write(dl_part, addr, buf, size) < 0) {
                LOG_E("Firmware download failed! Partition (%s) write data error!",
                    dl_part->name);
                return -RT_ERROR;
            }
            break;
...
download_buf_pop_last:
    ...
    ret = aic_ota_part_write(fhdr->begin_offset, (const uint8_t *)bhdr->buf, burn_len);
    if (ret)
        return -RT_ERROR;
