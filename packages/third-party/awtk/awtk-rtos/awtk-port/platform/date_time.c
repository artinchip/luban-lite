 ...
 * History:
 * ================================================================
 * 2023-10-24 Zequan Liang <zequan.liang@artinchip.com> created
 *
 */
...
static rt_device_t rtc_device;

static int tk_rtc_init(void)
{
  rtc_device = rt_device_find(RTC_NAME);
  if (!rtc_device) {
    log_error("find %s failed!", RTC_NAME);
    return -1;
  }

  if (rt_device_open(rtc_device, 0) != RT_EOK) {
    log_error("open %s failed!", RTC_NAME);
    return -1;
  }
