...
extern struct cli_commandset * get_default_cmds(void);
...
#define TOTAL_BLOCKS_DEFAULT			40
...
static void setup_storage(struct uffs_StorageAttrSt *attr)
{
	...
	memcpy(attr->_uffs_data_layout, MTD2K_LAYOUT_DATA, UFFS_SPARE_LAYOUT_SIZE);
	memcpy(attr->_uffs_ecc_layout, MTD2K_LAYOUT_ECC, UFFS_SPARE_LAYOUT_SIZE);
        ...
        if (arg[0] == '-') {
            if (!strcmp(arg, "-h") || !strcmp(arg, "--help")) {
				...
				if (++iarg > argc)
					...
					sprintf(script_command, "packdir %s", argv[iarg]);
					conf_exec_script = 1;
				}
			}
...
int main(int argc, char *argv[])
{
	...
	cli_add_commandset(get_default_cmds());
