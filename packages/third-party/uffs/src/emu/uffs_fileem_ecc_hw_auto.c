							...
							const u8 *data, int data_len, const u8 *ecc, const uffs_TagStore *ts)
{
	...
	if (data && data_len > 0) {
		...
		if (emu->em_monitor_page[abs_page] > PAGE_DATA_WRITE_COUNT_LIMIT) {
			MSG("Warning: block %d page %d exceed it's maximum write time!", block, page);
			goto err;
		}
	...
	if (ts) {
		...
		if (emu->em_monitor_spare[abs_page] > PAGE_SPARE_WRITE_COUNT_LIMIT) {
			MSG("Warning: block %d page %d (spare) exceed it's maximum write time!", block, page);
			goto err;
		}
									...
									uffs_TagStore *ts, u8 *ecc_store)
{
	...
	if (ts) {
		...
		if ((spare[SEAL_BYTE_OFFSET] == 0xFF) && (ret == UFFS_FLASH_NO_ERR)) {
