...
#define MAX_CLI_ARGS_BUF_LEN		1024
...
static int pack_dir(char *dir_path, char *dst_path)
{
	int ret = 0, len;
	DIR *dir;
	struct dirent *pd;
	...
	if (dir == NULL) {
		MSG("Failed to open directory %s\n", dir_path);
		return -1;
	}
	while ((pd = readdir(dir)) != NULL) {
		if (pd->d_type == DT_DIR) {
			...
			if (strcmp(".", pd->d_name) == 0)
				continue;
			sprintf(path_src, "%s/%s", dir_path, pd->d_name);
			sprintf(path_dst, "%s/%s", dst_path, pd->d_name);
			sprintf(cmd_buf, "mkdir %s", path_dst);
			ret = cli_interpret(cmd_buf);
			if (ret < 0)
				goto out;
			pack_dir(path_src, path_dst);
		}
		if (pd->d_type == DT_REG) {
			sprintf(path_src, "%s/%s", dir_path, pd->d_name);
			sprintf(path_dst, "%s/%s", dst_path, pd->d_name);
			sprintf(cmd_buf, "cp ::%s %s", path_src, path_dst);
			ret = cli_interpret(cmd_buf);
			if (ret < 0)
				goto out;
		}
...
/** pack files from local source directory
 *		packdir <dirpath>
 */
static int cmd_pack_dir(int argc, char *argv[])
{
	int ret = 0;
...
static const struct cli_command default_cmds[] = 
{
	...
	{ cmd_pack_dir,	"packdir",	"<src direcotry>",	"pack files from source directory", },
	{ NULL, NULL, NULL, NULL }
};
...
struct cli_commandset * get_default_cmds(void)
{
