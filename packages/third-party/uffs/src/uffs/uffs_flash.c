...
#define SEAL_BYTE(spare)  spare[SEAL_BYTE_OFFSET]	// seal byte is the 3 byte of spare data
							...
							int block, int page, uffs_Tags *tag)
{
	...
	if (ops->ReadPageWithLayout) {
		...
		if (tag) {
			tag->seal_byte = SEAL_BYTE(spare_buf);
						 ...
						 const uffs_TagStore *ts, const u8 *ecc, u8* spare)
{
	...
	SEAL_BYTE(spare) = 0;						// set seal byte = 0.
	...
	uffs_Assert(SEAL_BYTE(spare) == 0, "Make spare fail!");
}
