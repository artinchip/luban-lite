...
void deinit_mem_monitor(_list *pmem_table, int *used_num)
{
	...
	if(*used_num > 0)
		pr_info("Have %d mem_entry kept in monitor", *used_num);
	else
		pr_debug("No mem_entry kept in monitor");
	...
	while (rtw_end_of_queue_search(pmem_table, get_next(pmem_table)) == _FALSE)	{
		...
		pr_info("Not release memory at %p with size of %d", mem_entry->ptr, mem_entry->size);
...
void add_mem_usage(_list *pmem_table, void *ptr, int size, int *used_num, int flag)
{
...
#endif
	if(ptr == NULL) {
		pr_info("Catch a mem alloc fail with size of %d, current heap free size = %d", size, free_heap_size);
		return;
	}
	else{
		if(flag == MEM_MONITOR_FLAG_WPAS)
			pr_debug("Alloc memory at %p with size of %d", ptr, size);
		else
			pr_debug("Alloc memory at %p with size of %d", ptr, size);
	}
#if CONFIG_MEM_MONITOR & MEM_MONITOR_LEAK
	...
	if(mem_entry == NULL) {
		pr_info("Fail to alloc mem_entry");
		return;
	}
...
void del_mem_usage(_list *pmem_table, void *ptr, int *used_num, int flag)
{
	...
	if(plist == pmem_table)
		pr_info("Fail to find the mem_entry in mem table");
	else {
		*used_num --;
		_rtw_mfree((u8 *) mem_entry, sizeof(struct mem_entry));
	}
...
int get_mem_usage(_list *pmem_table)
{
	...
	if((plist = get_next(pmem_table)) == NULL) {
		pr_info("No mem table available\n");
		restore_flags();
		return 0;
	}
	...
	while (rtw_end_of_queue_search(pmem_table, plist) == _FALSE) {
		...
		pr_debug("size of mem_entry(%d)=%d\n", entry_num, mem_entry->size);
		plist = get_next(plist);
	}

	restore_flags();

	pr_debug("Get %d mem_entry\n", entry_num);
