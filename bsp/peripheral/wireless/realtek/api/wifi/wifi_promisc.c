...
static void promisc_test(int duration, unsigned char len_used)
{
	...
	for(ch = 1; ch <= 13; ch ++) {
		if(wifi_set_channel(ch) == 0)
			pr_info("Switch to channel(%d)", ch);
		...
		while(1) {
			...
			if(rtw_systime_to_ms(current_time - start_time) < duration) {
				...
				if(frame) {
					...
					pr_info("DA:");
					for(i = 0; i < 6; i ++)
						pr_info(" %02x", frame->da[i]);
					pr_info(", SA:");
					for(i = 0; i < 6; i ++)
						pr_info(" %02x", frame->sa[i]);
					pr_info(", len=%d", frame->len);
					pr_info(", RSSI=%d", frame->rssi);
					rtw_free((void *) frame);
				}
				else
...
void cmd_promisc(int argc, char **argv)
{
	...
	if((argc == 2) && ((duration = atoi(argv[1])) > 0))
		...
		pr_info("Usage: %s DURATION_SECONDS [with_len]", argv[0]);
}
