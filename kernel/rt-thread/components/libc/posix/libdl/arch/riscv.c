...
int dlmodule_relocate(struct rt_dlmodule *module, Elf_Rel *rel, Elf_Addr sym_val)
{
    ...
    switch (ELF32_R_TYPE(rel->r_info))
    {
        case R_RISCV_NONE:
            break;
        case R_RISCV_32:
            *where = (Elf32_Addr)(sym_val + rel->r_addend);
            RT_DEBUG_LOG(RT_DEBUG_MODULE, ("R_RISCV_32: %x -> %x\n",where, *where));
            break;
        case R_RISCV_RELATIVE:
            *where = (Elf32_Addr)((rt_uint8_t *)module->mem_space - module->vstart_addr + rel->r_addend);
            RT_DEBUG_LOG(RT_DEBUG_MODULE, ("R_RISCV_RELATIVE: %x -> %x\n",where, *where));
            break;
        case R_RISCV_JUMP_SLOT:
